function [redFiltImg] = form_h5_file(filename)
...
parfor_progress(length(image_times));
redFiltImg = zeros(movSize([1,2,4]),'single');
for ii=1:length(image_times)
    [greenImg,redImg] = get_single_volume(filename,volumes,lasers,piezo,img_idx,res,ii);
