function [greenImg,redImg]=get_single_volume(fname,volumes,lasers,piezo,img_idx,res,volume)
...
%vol_mem=img(:,:,:,volume,:);
    %if all(vol_mem(:)==0) %data not yet loaded
        ...
        redImg = zeros(res(1),res(2),length(find(lasers(frames,2))),'uint16');
        greenImg = zeros(res(1),res(2),length(find(lasers(frames,2))),'uint16');
