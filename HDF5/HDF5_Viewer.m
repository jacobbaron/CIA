function  HDF5_Viewer(load_all, fname, Img,disprange )
if ~exist('fname','var')
...
unique_piezo=sort(unique(piezo));
[~,z_idx]=ismember(piezo,unique_piezo);
Img=get_all_volumes(fname,lasers,piezo,img_idx,res,volumes,Img);
% if exist('load_all','var')
%     if load_all
%         h=waitbar(0,'Loading...');
%         tic
%        for ii=1:size(Img,4)
%           Img=get_volume(fname,ii,lasers,piezo,img_idx,res,volumes,Img);
%           waitbar(ii/size(Img,4),h);
%        end
%        toc
%         close(h);
%     end
% end
which_lasers=fliplr(find(fliplr(any(lasers,1))));
rgb=zeros([res(1),res(2),3]);
rgb(:,:,which_lasers)=double(squeeze(Img(:,:,Z,S,:)));
maxRGB=ones(size(rgb));
for ii=1:3
   if max(max(rgb(:,:,ii)))>0
        maxRGB(:,:,ii)=ones(res(1),res(2))*max(reshape(rgb(:,:,ii),[],1));
...
% -=< Slice slider callback function >=-
    function SliceSlider (hObj,event)
        ...
        rgb(:,:,which_lasers)=double(squeeze(Img(:,:,Z,S,:)))./maxRGB(:,:,which_lasers);
        image(rgb)
...
% -=< Window and level auto adjustment callback function >=-
    function AutoAdjust(object,eventdata)
        ...
        for ii=1:max(which_lasers)            
            
            m=max(max(double(squeeze(Img(:,:,Z,S,ii)))));
            if m==0
                m=1;
            end
            maxRGB(:,:,which_lasers(ii))=ones(size(maxRGB(:,:,which_lasers(ii))))*m;
        end
        rgb(:,:,which_lasers)=double(squeeze(Img(:,:,Z,S,:)))./maxRGB(:,:,which_lasers);
        image(rgb)
